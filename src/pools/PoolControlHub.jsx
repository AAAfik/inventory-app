...
export default function PoolControlHub({ TH, lang = "en", isMobile = false, isAdmin = false }) {
 ...
 fontFamily: "'Playfair Display', Georgia, serif",
 }}>Pool Control</div> {!isMobile && <div style={{fontSize:13, color:TH.textMuted, marginTop:2}}>Pool profiles · dosage calculator · treatment logs · auto warehouse deduction
        </div>}
      </div> {stats && (
      ...
      <div style={{display:"flex", gap:6, marginBottom:20, borderBottom:`1px solid ${TH.border}`, paddingBottom:8, overflowX:"auto"}}> {tabs.map(t => {
 ...
 fontFamily: "inherit", whiteSpace: "nowrap",
      ...
      </div> {tab === "pools"  && <PoolsListTab  key={refreshKey}  TH={TH} isMobile={isMobile} isAdmin={isAdmin} onChanged={bump} />}
...
function Stat({ TH, label, value, sub, onClick }) {
 ...
 boxShadow: TH.cardGlow,
      ...
      <div style={{fontSize:20, fontWeight:800, color:TH.text, lineHeight:1, fontFamily:"monospace"}}>{value}</div> {sub && <div style={{fontSize:10, color:TH.textDim, marginTop:4}}>{sub}</div>}
    </div> );
}
