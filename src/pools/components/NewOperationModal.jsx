...
export default function NewOperationModal({ TH, lang = "en", isMobile, presetPoolId = null, onClose, onDone }) {
  ...
  return (
    <div style={{position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", zIndex:9999, display:"flex", alignItems:"center", justifyContent:"center", padding:20}}>
      <div style={{background:TH.bgCard, border:`1px solid ${TH.border}`, borderRadius:14, padding:20, width:"100%", maxWidth:640, maxHeight:"92vh", overflowY:"auto"}}>
        <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14}}>
          <div style={{fontSize:16, fontWeight:800, color:TH.text, fontFamily:"'Playfair Display', Georgia, serif"}}>
            ⚙️ New pool operation
          </div>
        ...
        <div style={{display:"grid", gridTemplateColumns:"1fr", gap:10, marginBottom:12}}>
          <div>
            <label style={lbl(TH)}>Pool *</label>
            <select value={poolId} onChange={e => setPoolId(e.target.value)} disabled={busy || !!presetPoolId} style={inp(TH)}>
        ...
        <div style={{marginBottom:12}}>
          <label style={lbl(TH)}>Operation type *</label>
          <div style={{display:"grid", gridTemplateColumns:"repeat(4, 1fr)", gap:6}}>
            {OP_TYPES.map(t => {
              const on = opType === t.k;
              return (
                <button key={t.k} onClick={() => setOpType(t.k)} disabled={busy} style={{
        ...
        {opType === 'chemical_dosing' && (
          <div style={{marginBottom:12, padding:10, background:TH.bgInput, borderRadius:10}}>
            <div style={{fontSize:11, fontWeight:800, color:selectedOp.color, marginBottom:8, textTransform:"uppercase", letterSpacing:"0.05em"}}>💧 Water measurements</div>
            <div style={{display:"grid", gridTemplateColumns:"1fr 1fr 1fr 1fr", gap:6}}>
              <div><label style={miniLbl(TH)}>pH before</label><input type="number" step="0.1" value={phBefore} onChange={e => setPhBefore(e.target.value)} disabled={busy} style={smInp(TH)} /></div>
              <div><label style={miniLbl(TH)}>pH after</label><input type="number" step="0.1" value={phAfter} onChange={e => setPhAfter(e.target.value)} disabled={busy} style={smInp(TH)} /></div>
              <div><label style={miniLbl(TH)}>Cl before</label><input type="number" step="0.1" value={chlBefore} onChange={e => setChlBefore(e.target.value)} disabled={busy} style={smInp(TH)} /></div>
              <div><label style={miniLbl(TH)}>Cl after</label><input type="number" step="0.1" value={chlAfter} onChange={e => setChlAfter(e.target.value)} disabled={busy} style={smInp(TH)} /></div>
            </div>
          </div>
        )}

        {/* Filter change details */}
        {opType === 'filter_change' && (
          <div style={{marginBottom:12, padding:10, background:TH.bgInput, borderRadius:10}}>
            <div style={{fontSize:11, fontWeight:800, color:selectedOp.color, marginBottom:8, textTransform:"uppercase", letterSpacing:"0.05em"}}>🔄 Filter details</div>
            <div style={{display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:6}}>
              <div>
                <label style={miniLbl(TH)}>New filter type</label>
                <select value={filterType} onChange={e => setFilterType(e.target.value)} disabled={busy} style={smInp(TH)}>
                  <option value="">—</option>
                  {FILTER_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                </select>
              </div>
              <div>
                <label style={miniLbl(TH)}>Model</label>
                <input value={filterModel} onChange={e => setFilterModel(e.target.value)} disabled={busy} style={smInp(TH)} />
              </div>
              <div>
                <label style={miniLbl(TH)}>Changed on</label>
                <input type="date" value={filterDate} onChange={e => setFilterDate(e.target.value)} disabled={busy} style={smInp(TH)} />
              </div>
            </div>
          </div>
        )}
        ...
        <div style={{marginBottom:12, padding:10, background:TH.bgInput, borderRadius:10}}>
          <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8}}>
            <div style={{fontSize:11, fontWeight:800, color:TH.text, textTransform:"uppercase", letterSpacing:"0.05em"}}>🧪 Chemicals used ({chemicals.length})</div>
            <button onClick={addChemical} disabled={busy} style={{background:"transparent", border:`1px solid ${TH.border}`, borderRadius:6, color:TH.textMuted, padding:"4px 10px", cursor:"pointer", fontSize:11, fontWeight:600, fontFamily:"inherit"}}>+ Add chemical</button>
          </div>
          {chemicals.map((c, i) => {
            const item = items.find(it => String(it.id) === String(c.item_id));
            return (
              <div key={i} style={{background:TH.bgCard, borderRadius:8, padding:8, marginBottom:6, display:"grid", gridTemplateColumns:"2fr 1.5fr 80px auto", gap:6, alignItems:"end"}}>
                <div>
                  <label style={miniLbl(TH)}>Item</label>
                  <select value={c.item_id} onChange={e => { updateChemical(i, 'item_id', e.target.value); loadStockForItem(e.target.value, i); }} disabled={busy} style={smInp(TH)}>
                    <option value="">— Select —</option>
                    {items.map(it => <option key={it.id} value={it.id}>{it.name} ({it.current_qty} {it.unit})</option>)}
                  </select>
                </div>
                <div>
                  <label style={miniLbl(TH)}>Warehouse</label>
                  <select value={c.warehouse_id} onChange={e => updateChemical(i, 'warehouse_id', e.target.value)} disabled={busy || !c.stockRows} style={smInp(TH)}>
                    <option value="">—</option>
                    {(c.stockRows || warehouses.map(w => ({ warehouse_id: w.id, warehouse_name: w.name, qty: 0 }))).map(r => (
                      <option key={r.warehouse_id} value={r.warehouse_id}>
                        {r.warehouse_name}{r.qty > 0 ? ` (${r.qty})` : ''}
                      </option>
                    ))}
                  </select>
                </div>
                <div>
                  <label style={miniLbl(TH)}>Qty {item?.unit ? `(${item.unit})` : ''}</label>
                  <input type="number" step="0.01" min="0" value={c.qty} onChange={e => updateChemical(i, 'qty', e.target.value)} disabled={busy} style={smInp(TH)} />
                </div>
                <button onClick={() => removeChemical(i)} disabled={busy} style={{background:"transparent", border:`1px solid ${TH.border}`, borderRadius:6, color:"#C43D3D", padding:"6px 10px", cursor:"pointer", fontSize:12}}>✕</button>
              </div>
            );
          })}
          {chemicals.length === 0 && <div style={{fontSize:11, color:TH.textDim, textAlign:"center", padding:8}}>No chemicals added</div>}
        </div>

        {/* Photos */}
        <div style={{marginBottom:12}}>
          <label style={lbl(TH)}>Photos ({photoFiles.length})</label>
          <div style={{display:"flex", gap:6, flexWrap:"wrap"}}>
            {photoPreviews.map((src, i) => (
              <div key={i} style={{position:"relative", flexShrink:0, borderRadius:8, overflow:"hidden", border:`1px solid ${TH.border}`, background:"#000"}}>
                <img src={src} alt="" style={{width:60, height:60, objectFit:"cover", display:"block"}} />
                <button onClick={() => removePhoto(i)} disabled={busy} style={{position:"absolute", top:2, right:2, background:"rgba(0,0,0,0.75)", border:"none", borderRadius:10, width:18, height:18, color:"#fff", cursor:"pointer", fontSize:10, padding:0, lineHeight:1}}>✕</button>
              </div>
            ))}
            <button onClick={() => fileRef.current?.click()} disabled={busy} style={{flexShrink:0, width:60, height:60, background:"transparent", border:`2px dashed ${TH.border}`, borderRadius:8, color:TH.textMuted, cursor:"pointer", fontSize:22, fontFamily:"inherit"}}>+</button>
