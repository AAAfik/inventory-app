...
const EQUIPMENT_TYPES = [
  ...
  { k: 'skimmer',     icon: '🌀',  label: 'Skimmer',      color: '#5B7A9A' },
  { k: 'other',       icon: '📦',  label: 'Other',        color: '#8f8f8f' },
];
...
export default function EquipmentList({ TH, poolId, isAdmin, onZoom }) {
  ...
  return (
    <div>
      ...
      {items.length === 0 ? (
        <div style={{padding:40, background:TH.bgCard, border:`1px dashed ${TH.border}`, borderRadius:12, color:TH.textMuted, textAlign:"center", fontSize:13}}>
          No equipment registered yet. {isAdmin && "Click \"Add equipment\" to start."}
        </div>
      ) : (
        <div style={{display:"flex", flexDirection:"column", gap:10}}>
          {items.map(it => {
            const type = EQUIPMENT_TYPES.find(t => t.k === it.equipment_type) || { icon: '📦', label: it.equipment_type, color: '#8f8f8f' };
              ...
              <div key={it.id} style={{
                  ...
                  <div style={{display:"flex", gap:10, alignItems:"center"}}>
                    ...
                    <div>
                      <div style={{fontSize:13, fontWeight:800, color:TH.text}}>
                        {it.label || type.label}
                        {it.label && <span style={{fontSize:10, color:TH.textMuted, fontWeight:400, marginLeft:6}}>· {type.label}</span>}
                      </div>
                      <div style={{fontSize:11, color:TH.textMuted, marginTop:2}}>
                        {[it.brand, it.model, it.serial_number && `SN: ${it.serial_number}`].filter(Boolean).join(' · ') || '—'}
                      </div>
                    </div>
                  </div>
                  {isAdmin && (
                    <div style={{display:"flex", gap:4}}>
                      <button onClick={() => setEditing({ row: it })} style={ghostBtn(TH)}>Edit</button>
                      <button onClick={() => deleteItem(it.id)} style={{...ghostBtn(TH), color:"#C43D3D"}}>Delete</button>
                    </div>
                  )}
                </div>

                {/* Type-specific info */}
                {it.equipment_type === 'filter' && it.filter_media && (
                  <div style={{fontSize:11, color:TH.text, marginBottom:6}}>
                    <b>Media:</b> {it.filter_media}
                  </div>
                )}

                {/* Service info */}
                {(it.last_service_date || it.next_service_date) && (
                  <div style={{display:"flex", gap:8, flexWrap:"wrap", marginBottom:6}}>
                    {it.last_service_date && <Chip TH={TH}>Last: {it.last_service_date}</Chip>}
                    {it.next_service_date && (
                      <span style={{
                        ...
                        {overdue ? "⚠ Overdue" : dueSoon ? "⏳ Due soon"  : "Next"}: {it.next_service_date}
                      </span>
                    )}
                    {it.install_date && <Chip TH={TH}>Installed: {it.install_date}</Chip>}
                  </div>
                )}

                {it.notes && (
                  <div style={{fontSize:11, color:TH.textMuted, padding:8, background:TH.bgInput, borderRadius:6, marginTop:4}}>
                    {it.notes}
                  </div>
                )}
                ...
                {it.photos && it.photos.length > 0 && (
                  <div style={{display:"flex", gap:4, marginTop:8, flexWrap:"wrap"}}>
                    {it.photos.map((url, i) => (
                      <img key={i} src={url} alt="" onClick={() => onZoom?.(url)} style={{width:60, height:60, objectFit:"cover", borderRadius:6, background:"#000", cursor:"pointer"}} loading="lazy" />
                    ))}
                  </div>
                )}
              </div>
      ...
      <div style={{background:TH.bgCard, border:`1px solid ${TH.border}`, borderRadius:14, padding:20, width:"100%", maxWidth:560, maxHeight:"92vh", overflowY:"auto"}}>
        <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14}}>
          <div style={{fontSize:16, fontWeight:800, color:TH.text, fontFamily:"'Playfair Display', Georgia, serif"}}>
            {isNew ? '➕ Add equipment' : '✏️ Edit equipment'}
          </div>
        ...
        <div style={{marginBottom:12}}>
          <label style={lbl(TH)}>Type *</label>
          <div style={{display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:6}}>
            {EQUIPMENT_TYPES.map(t => {
              const on = form.equipment_type === t.k;
              return (
                <button key={t.k} onClick={() => setForm({...form, equipment_type: t.k})} disabled={busy} style={{
        ...
        <div style={{marginBottom:10}}>
          <label style={lbl(TH)}>Label (e.g. "Main Pump", "Filter #2")</label>
          <input value={form.label} onChange={e => setForm({...form, label: e.target.value})} disabled={busy} style={inp(TH)} />
        </div>

        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:10}}>
          <div><label style={lbl(TH)}>Brand</label><input value={form.brand} onChange={e => setForm({...form, brand: e.target.value})} disabled={busy} style={inp(TH)} /></div>
          <div><label style={lbl(TH)}>Model</label><input value={form.model} onChange={e => setForm({...form, model: e.target.value})} disabled={busy} style={inp(TH)} /></div>
        </div>

        <div style={{display:"grid", gridTemplateColumns:"2fr 1fr", gap:10, marginBottom:10}}>
          <div><label style={lbl(TH)}>Serial number</label><input value={form.serial_number} onChange={e => setForm({...form, serial_number: e.target.value})} disabled={busy} style={inp(TH)} /></div>
          <div><label style={lbl(TH)}>Install date</label><input type="date" value={form.install_date} onChange={e => setForm({...form, install_date: e.target.value})} disabled={busy} style={inp(TH)} /></div>
        </div>

        {form.equipment_type === 'filter' && (
          <div style={{marginBottom:10}}>
            <label style={lbl(TH)}>Filter media</label>
            <select value={form.filter_media} onChange={e => setForm({...form, filter_media: e.target.value})} disabled={busy} style={inp(TH)}>
              <option value="">—</option>
              {FILTER_MEDIA.map(m => <option key={m} value={m}>{m}</option>)}
            </select>
          </div>
        )}

        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:10}}>
          <div><label style={lbl(TH)}>Last service</label><input type="date" value={form.last_service_date} onChange={e => setForm({...form, last_service_date: e.target.value})} disabled={busy} style={inp(TH)} /></div>
          <div><label style={lbl(TH)}>Next service</label><input type="date" value={form.next_service_date} onChange={e => setForm({...form, next_service_date: e.target.value})} disabled={busy} style={inp(TH)} /></div>
          <div><label style={lbl(TH)}>Interval (days)</label><input type="number" min="0" value={form.service_interval_days} onChange={e => setForm({...form, service_interval_days: e.target.value})} disabled={busy} style={inp(TH)} /></div>
        </div>

        <div style={{marginBottom:12}}>
          <label style={lbl(TH)}>Photos</label>
          <div style={{display:"flex", gap:6, flexWrap:"wrap"}}>
            {photos.map((url, i) => (
              <div key={"e"+i} style={{position:"relative", borderRadius:8, overflow:"hidden", border:`1px solid ${TH.border}`, background:"#000"}}>
                <img src={url} alt="" style={{width:80, height:80, objectFit:"cover", display:"block"}} />
                <button onClick={() => removeExisting(i)} disabled={busy} style={{position:"absolute", top:2, right:2, background:"rgba(0,0,0,0.75)", border:"none", borderRadius:10, width:20, height:20, color:"#fff", cursor:"pointer", fontSize:11, padding:0, lineHeight:1}}>✕</button>
              </div>
            ))}
        ...
        <div style={{marginBottom:14}}>
          ...
          <textarea value={form.notes} onChange={e => setForm({...form, notes: e.target.value})} disabled={busy} rows={2} style={{...inp(TH), resize:"vertical"}} placeholder="Optional" />
        </div>
