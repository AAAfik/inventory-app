// ═══════════════════════════════════════════════════════════════════════
// LogTreatmentTab.jsx — mobile-first operator flow to log a treatment
// Pool → water readings → chemicals used → photo → save
// Auto-deducts from warehouse via DB trigger
// ═══════════════════════════════════════════════════════════════════════

import { useState, useEffect, useRef } from "react";
import { supabase } from "../../supabase";
import { POOL_TYPES, CHEMICAL_PURPOSES, CLARITY_OPTIONS, recommendedDose, estimatedCost, phStatus, chlorineStatus, fmtQty, fmtMoney, nextTreatmentNo } from "../lib/poolUtils";
...
export default function LogTreatmentTab({ TH, isMobile, onSaved }) {
  ...
  async function loadAll() {
    ...
    try {
      ...
      const [rP, rC, rW, rProp] = await Promise.all([
  ...
  function toggleChemical(chemId) {
    setLines(prev => {
      ...
      // Auto-fill with recommended dose
  ...
  async function submit() {
    ...
    try {
      ...
      // Insert lines — trigger auto-deducts from warehouse
  ...
  const totalEstCost = lines.reduce((sum, l) => {
    const chem = chemicals.find(c => c.id === l.chemical_id);
    return sum + (estimatedCost(chem, l.qty) || 0);
  }, 0);
  ...
  if (success) {
    return (
      <div style={{padding:"40px 20px", textAlign:"center", background:TH.bgCard, border:`1px solid ${TH.border}`, borderRadius:16}}>
        ...
        <div style={{fontSize:14, color:TH.text, fontFamily:"monospace", marginBottom:8}}>{success}</div>
  ...
  return (
    <div>
      ...
      <SectionCard TH={TH} title="Treatment header">
        <div style={{display:"grid", gridTemplateColumns:isMobile?"1fr":"1fr 1fr", gap:10}}>
          <Field TH={TH} label="Pool *">
            <select value={poolId} onChange={e => { setPoolId(e.target.value); setLines([]); }} style={inp(TH)}>
              ...
              {pools.map(p => {
                ...
                return <option key={p.id} value={p.id}>{type.icon} {p.name} ({p.volume_m3} m³)</option>;
              })}
            </select>
          </Field>
          ...
          <Field TH={TH} label="Withdraw chemicals from">
            <select value={warehouseId} onChange={e => setWarehouseId(e.target.value)} style={inp(TH)}>
              {warehouses.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}
            </select>
          </Field>
        </div>
        {selectedPool && (
          <div style={{marginTop:10, padding:10, background:"rgba(201,169,96,0.08)", borderRadius:8, fontSize:12, color:TH.text}}>
      ...
      <SectionCard TH={TH} title="Water readings (optional)">
        <div style={{display:"grid", gridTemplateColumns:isMobile?"1fr 1fr":"repeat(4, 1fr)", gap:10}}>
          <Field TH={TH} label="pH before">
            <input type="number" step="0.1" value={phBefore} onChange={e => setPhBefore(e.target.value)} placeholder="7.4" style={inp(TH)} />
            {phBefore && <StatusRow s={phStatus(phBefore)} />}
          </Field>
          <Field TH={TH} label="pH after">
            <input type="number" step="0.1" value={phAfter} onChange={e => setPhAfter(e.target.value)} placeholder="7.4" style={inp(TH)} />
            {phAfter && <StatusRow s={phStatus(phAfter)} />}
          </Field>
          <Field TH={TH} label="Chlorine ppm">
            <input type="number" step="0.1" value={chlorine} onChange={e => setChlorine(e.target.value)} placeholder="2.0" style={inp(TH)} />
            {chlorine && <StatusRow s={chlorineStatus(chlorine)} />}
          </Field>
        ...
        <div style={{marginTop:10}}>
          ...
          <div style={{display:"grid", gridTemplateColumns:"repeat(4, 1fr)", gap:6}}>
            {Object.entries(CLARITY_OPTIONS).map(([k, v]) => {
              const on = clarity === k;
              return (
      ...
      {/* Chemicals — grouped by purpose */}
      <SectionCard TH={TH} title="Chemicals used *">
        {!selectedPool && <div style={{padding:10, fontSize:12, color:TH.textDim, textAlign:"center"}}>Select a pool first to see recommended doses.</div>}
        {selectedPool && (
          <div style={{display:"grid", gridTemplateColumns:isMobile?"1fr":"repeat(auto-fill, minmax(220px, 1fr))", gap:8}}>
            {chemicals.map(chem => {
              ...
              const selected = lines.find(l => l.chemical_id === chem.id);
              const recDose = recommendedDose(chem, selectedPool.volume_m3);
              const cost = estimatedCost(chem, selected?.qty || 0);
              return (
                <div key={chem.id} style={{
                  background: selected ? "rgba(201,169,96,0.08)" : TH.bgInput,
                  border: `2px solid ${selected ? purp.color || "#C9A960" : TH.border}`,
                  borderRadius: 10, padding: 10, cursor: "pointer",
                }}
                onClick={(e) => { if (e.target.tagName !== 'INPUT') toggleChemical(chem.id); }}>
                  <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6}}>
                    <div style={{fontSize:12, fontWeight:700, color:TH.text}}>{selected ? '✓ ' : ''}{chem.name}</div>
                    <span style={{fontSize:16}}>{purp.icon}</span>
                  </div>
                  <div style={{fontSize:10, color:TH.textMuted, marginBottom:6}}>{purp.label} · rec {fmtQty(recDose, chem.unit)}</div>
                  {selected && (
                    <div style={{display:"flex", gap:6, alignItems:"center"}}>
                      <input
                        type="number" step="0.1" value={selected.qty}
                        onChange={e => updateLineQty(chem.id, e.target.value)}
                        onClick={e => e.stopPropagation()}
                        style={{...inp(TH), fontSize:14, fontWeight:700, padding:"6px 8px", width:80}}
                      />
                      <span style={{fontSize:11, color:TH.textMuted}}>{chem.unit}</span>
                      <span style={{marginLeft:"auto", fontSize:11, color:TH.accent, fontWeight:700, fontFamily:"monospace"}}>{fmtMoney(cost)}</span>
                    </div>
                  )}
                </div>
              );
        ...
        {lines.length > 0 && (
          <div style={{marginTop:12, padding:10, background:"rgba(201,169,96,0.10)", borderRadius:8, display:"flex", justifyContent:"space-between", fontSize:13}}>
            <span style={{color:TH.text, fontWeight:600}}>Total estimated cost:</span>
            <span style={{color:TH.accent, fontWeight:800, fontFamily:"monospace"}}>{fmtMoney(totalEstCost)}</span>
          </div>
        )}
      </SectionCard>
      ...
      <SectionCard TH={TH} title="Notes">
        <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3} placeholder="Observations, backwashing done, etc." style={{...inp(TH), resize:"vertical", minHeight:70}} />
      </SectionCard>
      ...
      {progress && (
        <div style={{background:TH.bgCard, border:`1px solid ${TH.border}`, borderRadius:10, padding:10, marginBottom:10}}>
          ...
          <div style={{height:4, background:TH.bgInput, borderRadius:2, overflow:"hidden"}}>
            <div style={{height:"100%", width:`${(progress.done/progress.total)*100}%`, background:"linear-gradient(90deg,#C9A960,#D4B876)"}}/>
          </div>
        </div>
      )}
...
function StatusRow({ s }) {
  if (!s) return null;
  return <div style={{marginTop:4, fontSize:10, fontWeight:700, color: s.color}}>● {s.label}{s.urgent ? " ⚠" : ""}</div>;
}
