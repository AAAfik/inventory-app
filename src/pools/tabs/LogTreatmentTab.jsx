  ...
  return (
    <div>
      ...
      <SectionCard TH={TH} title="Chemicals used *">
        {!selectedPool && <div style={{padding:20, fontSize:12, color:TH.textDim, textAlign:"center"}}>Select a pool first to see recommended doses.</div>}
        {selectedPool && (
              ...
              return (
                <div key={chem.id} onClick={(e) => { if (e.target.tagName !== 'INPUT') toggleChemical(chem.id); }} style={{
                  ...
                  <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6}}>
                    ...
                    <span style={{fontSize:18}}>{purp.icon}</span>
                  </div>
                  <div style={{fontSize:10, color:TH.textMuted, marginBottom:selectedLine?8:0}}>{purp.label} · rec {fmtQty(recDose, chem.unit)}</div>
                  {selectedLine && (
...
function SectionCard({ TH, title, children }) {
  return (
    <div style={{background:TH.bgCard, border:`1px solid ${TH.border}`, borderRadius:12, padding:14, marginBottom:12, boxShadow: TH.cardGlow}}>
      {title && <div style={{fontSize:12, fontWeight:800, color:TH.textMuted, textTransform:"uppercase", letterSpacing:"0.6px", marginBottom:10}}>{title}</div>}
      {children}
    </div>
  );
}
