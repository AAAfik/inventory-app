// ═══════════════════════════════════════════════════════════════════
// ChemicalStockTab.jsx — chemicals catalog + per-warehouse stock
// Shows dose per m³, cost, monthly usage/cost, current stock per warehouse
// ═══════════════════════════════════════════════════════════════════
...
export default function ChemicalStockTab({ TH, isMobile, isAdmin }) {
 ...
 const [rC, rW, rS, rI] = await Promise.all([
 ...
 setStock(rS.data || []);
 setItems(rI.data || []);
 // Aggregate this month's usage per chemical
 ...
 {editing && (
        <div style={{background:TH.bgCard, border:`2px solid ${TH.accentBorder}`, borderRadius:14, padding:18, marginBottom:16, boxShadow: TH.cardGlow}}>
          <div style={{fontSize:15, fontWeight:800, color:TH.text, marginBottom:12, fontFamily:"'Playfair Display', Georgia, serif"}}>Edit: {editing.name}</div>
          <div style={{display:"grid", gridTemplateColumns:isMobile?"1fr":"1fr 1fr", gap:10}}>
            <Field TH={TH} label="Name">
              <input value={editing.name} onChange={e => setEditing({...editing, name: e.target.value})} style={inp(TH)} />
            </Field>
            <Field TH={TH} label={`Dose per m³ (${editing.unit})`}>
              <input type="number" step="0.1" value={editing.dosage_per_m3 || ''} onChange={e => setEditing({...editing, dosage_per_m3: e.target.value})} style={inp(TH)} />
            </Field>
            <Field TH={TH} label={`Cost per ${editing.unit} (EUR)`}>
              <input type="number" step="0.001" value={editing.unit_cost || ''} onChange={e => setEditing({...editing, unit_cost: e.target.value})} style={inp(TH)} />
            </Field>
            <Field TH={TH} label="Min stock alert">
              <input type="number" value={editing.min_stock_alert || ''} onChange={e => setEditing({...editing, min_stock_alert: e.target.value})} style={inp(TH)} />
            </Field>
            <Field TH={TH} label="Linked warehouse item (for auto-deduct)">
              <select value={editing.item_id || ''} onChange={e => setEditing({...editing, item_id: e.target.value})} style={inp(TH)}>
                <option value="">— not linked (no auto-deduct) —</option> {items.map(it => <option key={it.id} value={it.id}>{it.code} · {it.name}</option>)}
              </select>
            </Field>
          </div>
 ...
 {loading ? (
        <div style={{padding:30, textAlign:"center", color:TH.textMuted}}>Loading...</div> ) : chemicals.length === 0 ? (
        <div style={{padding:40, background:TH.bgCard, border:`1px dashed ${TH.border}`, borderRadius:12, color:TH.textMuted, textAlign:"center"}}>No chemicals configured. Run the SQL migration to seed defaults.
        </div> ) : (
        <div style={{display:"flex", flexDirection:"column", gap:12}}> {chemicals.map(chem => {
 ...
 return (
                  ...
                  <div style={{padding:12, fontSize:11, color:TH.textDim, textAlign:"center", fontStyle:"italic", background:TH.bgInput, borderRadius:8}}>Not linked to any warehouse item — chemical use won't auto-deduct.
 {isAdmin && <span>Click "Edit" to link it.</span>}
                  </div> )}
              </div> );
 })}
        </div> )}
...
function th(TH) { return { padding:"6px 8px", fontSize:9, color:TH.textMuted, textTransform:"uppercase", letterSpacing:"0.6px", fontWeight:700, textAlign:"left" }; }
