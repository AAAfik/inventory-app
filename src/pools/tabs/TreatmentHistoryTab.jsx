...
export default function TreatmentHistoryTab({ TH, isMobile, isAdmin }) {
    ...
    <div>
      <div style={{display:"grid", gridTemplateColumns:isMobile?"1fr":"2fr 1fr 1fr", gap:8, marginBottom:16}}>
        ...
        <select value={poolFilter} onChange={e => setPoolFilter(e.target.value)} style={inp(TH)}>
          <option value="all">All pools</option> {pools.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
        </select>
 ...
 {loading ? (
        ...
        <div style={{padding:40, background:TH.bgCard, border:`1px dashed ${TH.border}`, borderRadius:12, color:TH.textMuted, textAlign:"center"}}>No treatments in this period.
        </div> ) : (
        <div style={{display:"flex", flexDirection:"column", gap:8}}> {filt.map(t => {
 const pool = poolMap[t.pool_id];
 ...
 onMouseLeave={e => e.currentTarget.style.background = TH.bgCard}> {cover ? (
                  <img src={cover} alt="" style={{width:52, height:52, objectFit:"cover", borderRadius:8, background:"#000", flexShrink:0}} loading="lazy" /> ) : (
                  <div style={{width:52, height:52, background:TH.bgInput, borderRadius:8, display:"flex", alignItems:"center", justifyContent:"center", fontSize:24, flexShrink:0}}></div> )}
...
function TreatmentDetail({ TH, isMobile, isAdmin, treatment, pool, onClose }) {
 ...
 return (
    <div> {zoom && (
      ...
      <div style={{display:"flex", justifyContent:"space-between", marginBottom:14, gap:8, flexWrap:"wrap"}}>
        ...
        <div style={{display:"flex", gap:6, alignItems:"center"}}> {isAdmin && <button onClick={deleteTreatment} style={{...ghostBtn(TH), color:"#8f8f8f"}}>Delete</button>}
 ...
 {treatment.photos?.length > 0 && (
        <div style={{background:TH.bgCard, border:`1px solid ${TH.border}`, borderRadius:14, padding:14, marginBottom:14, boxShadow: TH.cardGlow}}>
          ...
          <div style={{display:"grid", gridTemplateColumns:isMobile?"repeat(2, 1fr)":"repeat(auto-fill, minmax(150px, 1fr))", gap:8}}> {treatment.photos.map((url, i) => (
 ...
 {/* Chemicals used */}
      <div style={{background:TH.bgCard, border:`1px solid ${TH.border}`, borderRadius:14, padding:14, marginBottom:14, boxShadow: TH.cardGlow}}>
        <div style={{fontSize:12, fontWeight:800, color:TH.textMuted, textTransform:"uppercase", marginBottom:10, letterSpacing:"0.6px"}}>Chemicals used</div> {loading ? (
          <div style={{padding:20, textAlign:"center", color:TH.textDim}}>Loading...</div> ) : lines.length === 0 ? (
