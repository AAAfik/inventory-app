...
export default function App() {
  ...
  const allTabs = isAdmin ? ["dashboard","itemsMgmt","inventory","purchases","consumption","returns","orders","caesarMap","reports","suppliers","activityLog"] : ["consumption"];
          ...
          {tab==="caesarMap"&&isAdmin&&<>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20,flexWrap:"wrap",gap:10}}>
              ...
              <select value={mapCategoryFilter} onChange={e=>setMapCategoryFilter(e.target.value)} style={{background:TH.bgInput,border:`1px solid ${TH.borderStrong}`,borderRadius:9,padding:"9px 14px",color:TH.text,fontSize:13,outline:"none",fontFamily:"inherit"}}>
                ...
                {[...new Set(resortLocations.map(l=>l.category).filter(Boolean))].map(c=><option key={c} value={c}>{c} ({resortLocations.filter(l=>l.category===c).length})</option>)}
              </select>
            </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:isMobile?"repeat(2,1fr)":"repeat(auto-fit,minmax(160px,1fr))",gap:12,marginBottom:20}}>
              <div style={{...card,padding:14,textAlign:"center"}}>
                <div style={{color:TH.accent,fontSize:24,fontWeight:800}}>{resortLocations.length}</div>
                <div style={{color:TH.textMuted,fontSize:10,marginTop:4,textTransform:"uppercase",letterSpacing:"0.05em"}}>{t.mapTotalLocations}</div>
              </div>
              <div style={{...card,padding:14,textAlign:"center"}}>
                <div style={{color:"#22d3ee",fontSize:24,fontWeight:800}}>{[...new Set(resortLocations.map(l=>l.category).filter(Boolean))].length}</div>
                <div style={{color:TH.textMuted,fontSize:10,marginTop:4,textTransform:"uppercase",letterSpacing:"0.05em"}}>{t.mapCategories}</div>
              </div>
              <div style={{...card,padding:14,textAlign:"center"}}>
                <div style={{color:"#f59e0b",fontSize:24,fontWeight:800}}>{(mapCategoryFilter==="All"?resortLocations:resortLocations.filter(l=>l.category===mapCategoryFilter)).length}</div>
                <div style={{color:TH.textMuted,fontSize:10,marginTop:4,textTransform:"uppercase",letterSpacing:"0.05em"}}>Showing</div>
              </div>
            </div>
            ...
            {resortLocations.length === 0 ? (
              <div style={{...card,padding:40,textAlign:"center"}}>
                ...
                <div style={{color:TH.textMuted,fontSize:14}}>{t.mapNoLocations}</div>
              </div>
            ) : (
              <>
                <div style={{...card,padding:0,overflow:"hidden",position:"relative",marginBottom:20}}>
                  <div style={{position:"relative",width:"100%",aspectRatio:"16/9",background:`url('/resort-aerial.jpg') center/cover, #1a2035`,backgroundSize:"cover",backgroundPosition:"center"}}>
                    <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,.15)"}}/>
                    {(mapCategoryFilter==="All"?resortLocations:resortLocations.filter(l=>l.category===mapCategoryFilter)).map(loc => {
                      const x = loc.map_x ?? 50;
                      const y = loc.map_y ?? 50;
                      const color = getCategoryColor(loc.category);
                      const isSelected = selectedPin?.id === loc.id;
                      return (
                        <button key={loc.id} onClick={()=>setSelectedPin(loc)} style={{position:"absolute",left:`${x}%`,top:`${y}%`,transform:"translate(-50%,-100%)",background:"none",border:"none",cursor:"pointer",padding:0,zIndex:isSelected?20:10}} title={loc.name}>
                          <div style={{width:isSelected?34:26,height:isSelected?34:26,borderRadius:"50% 50% 50% 0",background:color,transform:"rotate(-45deg)",border:"2px solid #fff",boxShadow:`0 4px 12px ${color}80, 0 0 0 ${isSelected?4:0}px ${color}40`,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:isSelected?12:10,fontWeight:800,transition:"all .15s"}}>
                ...
                <div style={{...card,marginBottom:20}}>
                  ...
                  <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
                    {[...new Set(resortLocations.map(l=>l.category).filter(Boolean))].map(cat=>{
                ...
                <div style={card}>
                  <div style={cardTitle}><span>All Locations ({(mapCategoryFilter==="All"?resortLocations:resortLocations.filter(l=>l.category===mapCategoryFilter)).length})</span></div>
                  <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"repeat(auto-fill,minmax(280px,1fr))",gap:10}}>
                    {(mapCategoryFilter==="All"?resortLocations:resortLocations.filter(l=>l.category===mapCategoryFilter)).map(loc=>{
                      const color = getCategoryColor(loc.category);
                      return (
                        <button key={loc.id} onClick={()=>setSelectedPin(loc)} style={{display:"flex",alignItems:"flex-start",gap:10,background:TH.bgInput,border:`1px solid ${TH.border}`,borderRadius:10,padding:"10px 12px",cursor:"pointer",fontFamily:"inherit",textAlign:"left",borderLeft:`3px solid ${color}`}}>
                          <div style={{width:28,height:28,borderRadius:7,background:color+"22",color,fontSize:11,fontWeight:800,display:"flex",alignItems:"center",justifyContent:"center",border:`1px solid ${color}55`,flexShrink:0}}>{loc.id}</div>
                          <div style={{flex:1,minWidth:0}}>
                            <div style={{color:TH.text,fontSize:13,fontWeight:600,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{loc.name}</div>
