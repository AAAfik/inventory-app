...
export default function App() {
  ...
  // ─── REPORT GENERATION (Maximum Transparency) ──────────────
  async function generateCustomReport() {
    ...
    const fmt = (iso) => iso ? new Date(iso).toLocaleString("en-GB",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",second:"2-digit"}) : "";
    const iso = (val) => val ? new Date(val).toISOString() : "";
    const generatedAtReadable = new Date().toLocaleString("en-GB",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",second:"2-digit"});
    const generatedAtIso = new Date().toISOString();

    const wb = XLSX.utils.book_new();

    // ═══ SHEET 1: REPORT INFO — Full Context ═══
    const summaryData = [
      {Field: "Report ID",              Value: `RPT-${Date.now()}`},
      {Field: "Report Generated (UTC)", Value: generatedAtIso},
      {Field: "Report Generated (Local)", Value: generatedAtReadable},
      {Field: "Generated By (Email)",   Value: user?.email || ""},
      {Field: "Generated By (Role)",    Value: isAdmin ? "Admin" : "Staff"},
      {Field: "Browser",                Value: navigator.userAgent.slice(0,200)},
      {Field: "Language",               Value: lang},
      {Field: "Sections Selected",      Value: selected.length},
      {Field: "Sections List",          Value: selected.join(", ")},
      {Field: "—",                      Value: "—"},
      {Field: "Total Items in System",  Value: items.length},
      {Field: "Total Purchases",        Value: purchases.length},
      {Field: "Approved Purchases",     Value: purchases.filter(p=>p.status==="approved").length},
      {Field: "Pending Purchases",      Value: purchases.filter(p=>p.status==="pending").length},
      {Field: "Rejected Purchases",     Value: purchases.filter(p=>p.status==="rejected").length},
      {Field: "Total Consumptions",     Value: consumptions.length},
      {Field: "Total Returns",          Value: returns.length},
      {Field: "Total Orders",           Value: orders.length},
      {Field: "Total Activity Logs",    Value: logs.length},
      {Field: "Total Stock Value (₺)",  Value: Math.round(inventory.reduce((s,i)=>s+(i.totalValue||0),0))},
      {Field: "Total Approved Spend (₺)", Value: Math.round(purchases.filter(p=>p.status==="approved").reduce((s,p)=>s+p.qty*p.unitPrice,0))},
    ];
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(summaryData), "Report Info");
    ...
    for (const key of selected) {
      ...
      if (key === "inventorySummary") {
        data = inventory.map(i => {
          const itemPurchases = purchases.filter(p=>p.itemId===i.id&&p.status==="approved");
          const firstP = itemPurchases.length ? [...itemPurchases].sort((a,b)=>(a.date||"").localeCompare(b.date||""))[0] : null;
          const lastP = itemPurchases.length ? [...itemPurchases].sort((a,b)=>(b.date||"").localeCompare(a.date||""))[0] : null;
          return {
            ItemID: i.id,
            Code: i.code,
            Name: i.name,
            ...
            Supplier: i.supplier||"",
            HasImage: i.image_url ? "Yes" : "No",
            QtyPurchased: i.purchased,
            QtyConsumed: i.consumed,
            QtyReturned: i.returned,
            CurrentStock: i.stock,
            ShortageQty: Math.max(0, i.min_stock - i.stock),
            AvgPrice: Math.round((i.avgPrice||0)*100)/100,
            LastPrice: lastP ? lastP.unitPrice : 0,
            TotalValue: Math.round(i.totalValue||0),
            Status: i.stock<=0 ? "OUT" : i.low ? "LOW" : "OK",
            FirstPurchaseDate: firstP?.date || "",
            LastPurchaseDate: lastP?.date || "",
            ItemCreatedAt: fmt(i.created_at),
            ItemCreatedAtISO: iso(i.created_at),
            ReportGeneratedAt: generatedAtReadable
          };
        });
        ...
        data = inventory.filter(i=>i.low||i.stock<=0).map(i => ({
          ...
          CurrentStock: i.stock, MinStock: i.min_stock,
          ShortageQty: Math.max(0, i.min_stock - i.stock),
          AvgPrice: Math.round((i.avgPrice||0)*100)/100,
          EstimatedRestockCost: Math.round(Math.max(0, i.min_stock - i.stock) * (i.avgPrice||0)),
        ...
        data = [...inventory].sort((a,b)=>b.totalValue-a.totalValue).slice(0,20).map((i,idx) => ({
          Rank: idx+1,
          ItemID: i.id, Code: i.code, Name: i.name,
          CurrentStock: i.stock, Unit: i.unit,
          AvgPrice: Math.round((i.avgPrice||0)*100)/100,
          TotalValue: Math.round(i.totalValue||0),
          PercentOfTotal: ((i.totalValue / inventory.reduce((s,x)=>s+(x.totalValue||0),0)) * 100).toFixed(2) + "%",
          Supplier: i.supplier||"",
          ItemCreatedAt: fmt(i.created_at),
          ReportGeneratedAt: generatedAtReadable
        }));
        ...
        data = purchases.map(p => {
          ...
          return {
            ...
            CreatedAt: fmt(p.created_at),
            CreatedAtISO: iso(p.created_at),
            ReportGeneratedAt: generatedAtReadable
          };
        });
        ...
        data = purchases.filter(p=>p.status==="approved").map(p => {
          const it = items.find(i=>i.id===p.itemId);
          return {
            PurchaseID: p.id, ItemID: p.itemId, ItemCode: it?.code||"", Product: it?.name||"",
            PurchaseDate: p.date||"", Qty: p.qty, Unit: it?.unit||"",
            ...
            Note: p.note||"", ApprovedBy: p.created_by||"",
            CreatedAt: fmt(p.created_at), CreatedAtISO: iso(p.created_at),
            ReportGeneratedAt: generatedAtReadable
          };
        });
        ...
        data = purchases.filter(p=>p.status==="pending").map(p => {
          ...
          return {
            ...
            RequestedBy: p.created_by||"", Note: p.note||"",
            CreatedAt: fmt(p.created_at), CreatedAtISO: iso(p.created_at),
            ReportGeneratedAt: generatedAtReadable
          };
        });
        ...
        data = consumptions.map(c => {
          ...
          return {
            ...
            Product: it?.name||"",
            Unit: it?.unit||"",
            ConsumptionDate: c.date||"",
            Qty: c.qty,
            EstimatedValue: Math.round(Number(c.qty) * (inv?.avgPrice||0)),
            Location: c.location||"",
            DeliveredTo: c.deliveredTo||"",
            DeliveredBy: c.deliveryPerson||"",
            Operator: c.operator||"",
            Note: c.note||"",
            LoggedBy: c.created_by||"",
            LoggedAt: fmt(c.created_at),
            LoggedAtISO: iso(c.created_at),
            ReportGeneratedAt: generatedAtReadable
          };
        });
        ...
        data = returns.map(r => {
          ...
          return {
            ...
            ReturnDate: r.date||"",
            Qty: r.qty,
            EstimatedValue: Math.round(Number(r.qty) * (inv?.avgPrice||0)),
            Reason: r.reason||"",
        ...
        consumptions.forEach(c => {
          ...
          if (!agg[dept]) agg[dept] = {Department: dept, Records: 0, UniqueItems: new Set(), UniqueOperators: new Set(), TotalQty: 0, EstimatedValue: 0, FirstUsage: c.date||"", LastUsage: c.date||"", FirstLoggedAt: c.created_at, LastLoggedAt: c.created_at};
          agg[dept].Records++;
          agg[dept].UniqueItems.add(c.itemId);
          agg[dept].UniqueOperators.add(c.operator||"—");
          agg[dept].TotalQty += Number(c.qty);
          agg[dept].EstimatedValue += Number(c.qty) * (inv?.avgPrice||0);
          if(c.date && c.date < agg[dept].FirstUsage) agg[dept].FirstUsage = c.date;
          if(c.date && c.date > agg[dept].LastUsage) agg[dept].LastUsage = c.date;
          if(c.created_at && c.created_at < agg[dept].FirstLoggedAt) agg[dept].FirstLoggedAt = c.created_at;
          if(c.created_at && c.created_at > agg[dept].LastLoggedAt) agg[dept].LastLoggedAt = c.created_at;
        });
        data = Object.values(agg).map(d => ({
          Department: d.Department,
          Records: d.Records,
          UniqueItems: d.UniqueItems.size,
          UniqueOperators: d.UniqueOperators.size,
          TotalQty: d.TotalQty,
          EstimatedValue: Math.round(d.EstimatedValue),
          FirstUsageDate: d.FirstUsage,
          LastUsageDate: d.LastUsage,
          FirstLoggedAt: fmt(d.FirstLoggedAt),
          LastLoggedAt: fmt(d.LastLoggedAt),
          ReportGeneratedAt: generatedAtReadable
        }));
        ...
        consumptions.forEach(c => {
          const inv = inventory.find(i=>i.id===c.itemId);
          const op = c.operator || "—";
          if (!agg[op]) agg[op] = {Operator: op, Records: 0, UniqueItems: new Set(), UniqueLocations: new Set(), TotalQty: 0, EstimatedValue: 0, FirstUsage: c.date||"", LastUsage: c.date||""};
          agg[op].Records++;
          agg[op].UniqueItems.add(c.itemId);
          agg[op].UniqueLocations.add(c.location||"—");
          agg[op].TotalQty += Number(c.qty);
          agg[op].EstimatedValue += Number(c.qty) * (inv?.avgPrice||0);
          if(c.date && c.date < agg[op].FirstUsage) agg[op].FirstUsage = c.date;
          if(c.date && c.date > agg[op].LastUsage) agg[op].LastUsage = c.date;
        });
        data = Object.values(agg).map(d => ({
          Operator: d.Operator,
          Records: d.Records,
          UniqueItems: d.UniqueItems.size,
          UniqueLocations: d.UniqueLocations.size,
          TotalQty: d.TotalQty,
          EstimatedValue: Math.round(d.EstimatedValue),
          FirstUsageDate: d.FirstUsage,
          LastUsageDate: d.LastUsage,
        ...
        data = SUPPLIERS_LIST.map(s => {
          ...
          const uniqueItems = new Set(sp.map(p=>p.itemId));
          return {
            Supplier: s,
            TotalPurchases: all.length,
            ApprovedPurchases: sp.length,
            PendingPurchases: all.filter(p=>p.status==="pending").length,
            RejectedPurchases: all.filter(p=>p.status==="rejected").length,
            UniqueItemsSupplied: uniqueItems.size,
            TotalQty: sp.reduce((sum,p)=>sum+Number(p.qty),0),
            TotalSpend: Math.round(sp.reduce((sum,p)=>sum+p.qty*p.unitPrice,0)),
            AvgPurchaseValue: sp.length ? Math.round(sp.reduce((sum,p)=>sum+p.qty*p.unitPrice,0)/sp.length) : 0,
        ...
        const totalRejected = purchases.filter(p=>p.status==="rejected").reduce((s,p)=>s+p.qty*p.unitPrice,0);
        const totalConsumed = consumptions.reduce((sum,c)=>{const inv=inventory.find(i=>i.id===c.itemId);return sum+Number(c.qty)*(inv?.avgPrice||0);},0);
        data = [
          {Metric: "Report Generated (Local)", Value: generatedAtReadable},
          {Metric: "Report Generated (UTC ISO)", Value: generatedAtIso},
          ...
          {Metric: "Total Rejected Spend (₺)", Value: Math.round(totalRejected)},
          {Metric: "Estimated Consumption Value (₺)", Value: Math.round(totalConsumed)},
          {Metric: "—", Value: "—"},
          {Metric: "Number of Items", Value: items.length},
          {Metric: "Items in Stock", Value: inventory.filter(i=>i.stock>0).length},
          {Metric: "Low Stock Items", Value: inventory.filter(i=>i.low).length},
          {Metric: "Out of Stock Items", Value: inventory.filter(i=>i.stock<=0).length},
          {Metric: "—", Value: "—"},
          ...
          {Metric: "Rejected Purchases", Value: purchases.filter(p=>p.status==="rejected").length},
          {Metric: "—", Value: "—"},
          ...
          {Metric: "Total Activity Logs", Value: logs.length},
          {Metric: "Number of Suppliers", Value: SUPPLIERS_LIST.length},
          {Metric: "Number of Departments", Value: DEPTS.length},
        ];
        ...
        data = discrepancies.map(d => ({
          ...
          DifferenceQty: (d.recent - d.avg).toFixed(2),
          SpikeRatio: (d.recent/d.avg).toFixed(2) + "x",
          Severity: d.recent/d.avg > 5 ? "HIGH" : d.recent/d.avg > 3 ? "MEDIUM" : "LOW",
          DetectedAt: generatedAtReadable
        }));
        sheetName = "Anomalies";
      }
        ...
        data = logs.map(l => ({
          LogID: l.id,
          DateTime: fmt(l.created_at),
          DateTimeISO: iso(l.created_at),
          Action: l.action,
          Details: l.details,
          User: l.user_email,
    ...
    const fname = `StockTrack_FullReport_${ts}.xlsx`;
    XLSX.writeFile(wb, fname);
    await logAction("Generated Report", `Reports: ${selected.join(", ")} sheets:${wb.SheetNames.length} at:${generatedAtReadable} by:${user?.email}`);
    alert(t.reportGenerated + " ✓\n\n" + wb.SheetNames.length + " sheets generated");
  }
