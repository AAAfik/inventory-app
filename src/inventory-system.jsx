...
const TAB_ICONS = {reports:"\ud83d\udccb",dashboard:"\u25c8",itemsMgmt:"\u229e",inventory:"\u25a6",purchases:"\u2193",consumption:"\u2197",returns:"\u21a9",orders:"\u25ce",suppliers:"\u22a1",activityLog:"\ud83d\udccb",caesarMap:"\ud83d\uddfa",pools:"\ud83c\udfca",procure:"\ud83d\udcb3",warehouse:"\ud83c\udfec",inspection:"\ud83d\udd0d",users:"\ud83d\udc65"};
...
export default function App() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      if (user) {
        ...
        setIsAdmin(admin);
        // Fetch user roles from procure.user_roles
        try {
          const { data: rolesData } = await supabase.rpc('my_roles', {}, { schema: 'procure' });
          setUserRoles(Array.isArray(rolesData) ? rolesData : []);
        } catch (e) {
          // Fallback: query directly
          const { data } = await supabase.schema('procure').from('user_roles').select('role').eq('user_id', user.id).eq('is_active', true);
  ...
  const isOwner = isAdmin || userRoles.includes("owner") || userRoles.includes("auditor");
  ...
  const canSeeProcure = isOwner || ["procurement_officer","deputy_officer","dept_head","operator","finance_officer","approver_mid","approver_high"].some(r => userRoles.includes(r));
  const canSeeUsers = isOwner; // Team management is owner-only
    ...
    <div dir={lang==="fa"||lang==="he"?"rtl":"ltr"} style={{display:"flex",flexDirection:"column",minHeight:"100vh",background:TH.bg,color:TH.text,fontFamily:lang==="fa"?"'Vazirmatn','Tahoma',sans-serif":lang==="he"?"'Heebo','Arial',sans-serif":"'Inter','Segoe UI',system-ui,sans-serif"}}>
      ...
      <div style={{display:"flex",flex:1,minHeight:0,position:"relative"}}>
        ...
        <main style={{flex:1,padding:isMobile?"16px 14px":"24px 28px",overflowY:"auto",overflowX:"hidden",minWidth:0,width:"100%"}}>
          ...
          {INSPECTION_ENABLED&&tab==="inspection"&&canSeeInspection&&<InspectionHub TH={TH} lang={lang} isMobile={isMobile} isAdmin={isOwner} />}

          {PROCURE_ENABLED&&tab==="procure"&&canSeeProcure&&<ProcureHub TH={TH} lang={lang} isMobile={isMobile} isAdmin={isOwner} />}
