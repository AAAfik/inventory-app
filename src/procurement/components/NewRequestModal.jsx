    ...
    <div style={{position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", zIndex:9999, display:"flex", alignItems:"center", justifyContent:"center", padding:20}}>
      <div style={{background:TH.bgCard, border:`1px solid ${TH.border}`, borderRadius:14, padding:20, width:"100%", maxWidth:640, maxHeight:"92vh", overflowY:"auto"}}>
        ...
        <div style={{display:"grid", gridTemplateColumns:"2fr 1fr", gap:10, marginBottom:10}}>
          <div>
            <label style={lbl(TH)}>{L.department || 'Department'} *</label>
            <select value={deptId} onChange={e => setDeptId(e.target.value)} disabled={busy} style={inp(TH)}>
              <option value="">—</option>
              {departments.map(d => <option key={d.id} value={d.id}>{d.icon} {d.name}</option>)}
            </select>
          </div>
          <div>
            <label style={lbl(TH)}>{L.priority || 'Priority'}</label>
            <select value={priority} onChange={e => setPriority(e.target.value)} disabled={busy} style={inp(TH)}>
        ...
        <div style={{marginBottom:10}}>
          <label style={lbl(TH)}>{L.property || 'Property'}</label>
        ...
        <div style={{marginBottom:10}}>
          <label style={lbl(TH)}>{L.purpose || 'Purpose / Reason'} *</label>
          <textarea value={purpose} onChange={e => setPurpose(e.target.value)} disabled={busy} rows={2} style={{...inp(TH), resize:"vertical"}} placeholder={L.purposePh || 'Why do you need this?'} />
        </div>

        <div style={{marginBottom:14}}>
          <label style={lbl(TH)}>{L.useLocation || 'Where will it be used?'}</label>
          <input value={useLocation} onChange={e => setUseLocation(e.target.value)} disabled={busy} style={inp(TH)} placeholder={L.useLocationPh || 'e.g. Caesar Cliff — main garden'} />
        </div>

        {/* Items */}
        <div style={{marginBottom:14, background:TH.bgInput, borderRadius:10, padding:12}}>
          <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10}}>
            <div style={{fontSize:12, fontWeight:700, color:TH.text, textTransform:"uppercase", letterSpacing:"0.5px"}}>
              {L.items || 'Items'} ({items.length})
            </div>
            <button onClick={addItem} disabled={busy} style={{background:"transparent", border:`1px solid ${TH.border}`, borderRadius:6, color:TH.textMuted, padding:"4px 10px", cursor:"pointer", fontSize:11, fontWeight:600, fontFamily:"inherit"}}>+ {L.addItem || 'Add item'}</button>
          </div>
          {items.map((it, i) => (
            <div key={i} style={{background:TH.bgCard, border:`1px solid ${TH.border}`, borderRadius:8, padding:10, marginBottom:8}}>
              <div style={{display:"grid", gridTemplateColumns:"1fr 60px 80px auto", gap:6, marginBottom:6, alignItems:"end"}}>
                <div>
                  <label style={miniLbl(TH)}>{L.itemName || 'Item name'}</label>
                  <input value={it.name} onChange={e => updateItem(i, 'name', e.target.value)} disabled={busy} placeholder="Shovel" style={smInp(TH)} />
                </div>
                <div>
                  <label style={miniLbl(TH)}>{L.qty || 'Qty'}</label>
                  <input type="number" step="0.1" min="0" value={it.qty} onChange={e => updateItem(i, 'qty', e.target.value)} disabled={busy} style={smInp(TH)} />
                </div>
                <div>
                  <label style={miniLbl(TH)}>{L.unit || 'Unit'}</label>
                  <input value={it.unit} onChange={e => updateItem(i, 'unit', e.target.value)} disabled={busy} placeholder="unit" style={smInp(TH)} />
                </div>
                {items.length > 1 && (
                  <button onClick={() => removeItem(i)} disabled={busy} style={{background:"transparent", border:`1px solid ${TH.border}`, borderRadius:6, color:"#C43D3D", padding:"7px 10px", cursor:"pointer", fontSize:14, fontFamily:"inherit"}}>✕</button>
                )}
              </div>
              <div style={{display:"grid", gridTemplateColumns:"120px 1fr", gap:6}}>
                <div>
                  <label style={miniLbl(TH)}>{L.estCost || 'Est. cost'}</label>
                  <input type="number" step="0.01" min="0" value={it.estimated_cost} onChange={e => updateItem(i, 'estimated_cost', e.target.value)} disabled={busy} placeholder="0.00" style={smInp(TH)} />
                </div>
                <div>
                  <label style={miniLbl(TH)}>{L.itemNote || 'Note'}</label>
                  <input value={it.notes} onChange={e => updateItem(i, 'notes', e.target.value)} disabled={busy} placeholder="Optional" style={smInp(TH)} />
                </div>
              </div>
            </div>
          ))}
        ...
        <div style={{display:"flex", gap:8, justifyContent:"flex-end"}}>
          ...
          <button onClick={submit} disabled={busy} style={{background:"linear-gradient(135deg,#B8935A,#8B7040)", border:"none", borderRadius:9, color:"#000", padding:"10px 24px", cursor:"pointer", fontSize:13, fontWeight:800, fontFamily:"inherit", opacity:busy?0.6:1}}>
            {busy ? (L.saving || 'Saving…') : (L.submitRequest || 'Submit request')}
          </button>
        </div>
      </div>
    </div>
