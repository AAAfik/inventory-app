...
//       Loans · Packages · Warehouses · Activity
// + Receive + Dispense buttons in header (consumable quick actions)
// ═══════════════════════════════════════════════════════════════════
...
export default function WarehouseHub({ TH, lang = "en", isMobile = false, isAdmin = false }) {
  ...
  const tabs = [
    { key: "scan", label: L.scanTab       || "Scan" },
    { key: "quickadd", label: L.quickAdd },
    { key: "assets", label: L.assets },
    { key: "checkinout",  label: L.checkInOut, badge: stats?.overdueReturns || 0 },
    { key: "consumables", label: L.consumables },
    { key: "loans", label: L.loansTab || "Loans", badge: stats?.overdueReturns || 0, alert: (stats?.overdueReturns || 0) > 0 },
    { key: "packages", label: L.packagesTab || "Packages", badge: stats?.pendingPackages || 0 },
    { key: "warehouses", label: L.warehouses },
    { key: "activity", label: L.activityTab || "Activity" },
  ];
  ...
  return (
    <div>
      {showDispense && (
        <DispenseModal TH={TH} lang={lang}
          onClose={() => setShowDispense(false)}
          onDone={() => { setShowDispense(false); bump(); }} />
      )}
      {showReceive && (
        <ReceiveModal TH={TH} lang={lang}
          onClose={() => setShowReceive(false)}
          onDone={() => { setShowReceive(false); bump(); }} />
      )}

      <div style={{marginBottom:14, display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:12, flexWrap:"wrap"}}><div><div style={{fontSize:isMobile?18:24, fontWeight:700, color:TH.text, letterSpacing:"-0.3px", fontFamily:"'Playfair Display', Georgia, serif"}}>{title}</div>
          {!isMobile && subtitle && <div style={{fontSize:13, color:TH.textMuted, marginTop:2}}>{subtitle}</div>}
        </div><div style={{display:"flex", gap:8, flexWrap:"wrap"}}><button onClick={() => setShowReceive(true)} title={L.receiveTitle || "Receive stock"} style={{
            background:TH.ok, border:"none", borderRadius:10,
            color:TH.onDeep, padding:"10px 18px", fontSize:13, fontWeight:800, cursor:"pointer",
            fontFamily:"inherit", display:"inline-flex", alignItems:"center", gap:6,
          }}><Icon name="arrowDown" size={15} />{L.receiveBtn || "Receive"}</button><button onClick={() => setShowDispense(true)} title={L.dispenseTitle || "Dispense consumable"} style={{
            background:TH.deep, border:"none", borderRadius:10,
            color:TH.onDeep, padding:"10px 18px", fontSize:13, fontWeight:800, cursor:"pointer",
            fontFamily:"inherit", display:"inline-flex", alignItems:"center", gap:6,
          }}><Icon name="arrowUp" size={15} />{L.dispenseBtn || "Dispense"}</button></div></div>

      {stats && (
        <div style={{display:"grid", gridTemplateColumns:isMobile?"repeat(2,1fr)":"repeat(6,1fr)", gap:8, marginBottom:16}}><Stat TH={TH} label={L.assets} value={stats.total} /><Stat TH={TH} label={L.totalValue} value={fmtMoney(stats.totalValue)} /><Stat TH={TH} label={L.checkedOutK} value={stats.checkedOut} onClick={() => setTab("loans")} /><Stat TH={TH} label={L.overdueReturns} value={stats.overdueReturns} alert={stats.overdueReturns > 0} onClick={() => setTab("loans")} /><Stat TH={TH} label={L.serviceDue7} value={stats.serviceDue} alert={stats.serviceDue > 0} onClick={() => setTab("assets")} /><Stat TH={TH} label={L.pendingPackages || "Pending pkgs"} value={stats.pendingPackages} alert={stats.pendingPackages > 0} onClick={() => setTab("packages")} /></div>
      )}
      ...
      {tab === "consumables" && <ConsumablesTab key={"cn-"+refreshKey} TH={TH} lang={lang} isMobile={isMobile} isAdmin={isAdmin} />}
...
function Stat({ TH, label, value, alert, onClick }) {
  return (
    ...
    }}><div style={{fontSize:9, fontWeight:700, color:TH.textMuted, textTransform:"uppercase", letterSpacing:"0.5px", marginBottom:4}}>{label}</div><div style={{fontSize:20, fontWeight:800, color: alert ? "#C43D3D" : TH.text, lineHeight:1}}>{value}</div></div>
  );
}
