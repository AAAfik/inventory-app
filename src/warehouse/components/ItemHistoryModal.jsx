...
export default function ItemHistoryModal({ TH, lang = "en", item, onClose }) {
  ...
  return (
    <div style={{position:"fixed", inset:0, background:"rgba(0,0,0,0.9)", zIndex:9999, display:"flex", alignItems:"center", justifyContent:"center", padding:20}}>
      <div style={{background:TH.bgCard, border:`1px solid ${TH.border}`, borderRadius:14, padding:20, width:"100%", maxWidth:960, maxHeight:"92vh", display:"flex", flexDirection:"column"}}>

        {/* Header */}
        <div style={{display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:14, gap:12}}>
          <div>
            <div style={{fontSize:18, fontWeight:800, color:TH.text, fontFamily:"'Playfair Display', Georgia, serif"}}>{item.name}</div>
        ...
        <div style={{display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:8, marginBottom:12}}>
          <div style={sumCard(TH, "#7A9A5B")}>
            <div style={sumLbl(TH)}>Total IN</div>
            <div style={sumVal(TH)}>+{totalIn.toFixed(2)} <span style={{fontSize:11, fontWeight:500, color:TH.textDim}}>{item.unit}</span></div>
          </div>
          <div style={sumCard(TH, "#C43D3D")}>
            <div style={sumLbl(TH)}>Total OUT</div>
            <div style={sumVal(TH)}>-{totalOut.toFixed(2)} <span style={{fontSize:11, fontWeight:500, color:TH.textDim}}>{item.unit}</span></div>
          </div>
          <div style={sumCard(TH, "#B8935A")}>
            <div style={sumLbl(TH)}>Spent (IN)</div>
            <div style={sumVal(TH)}>€{totalCostIn.toFixed(2)}</div>
          </div>
        </div>
                ...
                return (
                  <div key={r.id} style={{
                    ...
                    <div style={{minWidth:64, textAlign:"center"}}>
                      <div style={{fontSize:20, fontWeight:800, color:stateColor, lineHeight:1}}>
                        {isIn ? '+' : ''}{r.qty}
                      </div>
                      <div style={{fontSize:9, color:TH.textDim, marginTop:2}}>{isIn ? 'IN' : 'OUT'}</div>
                    </div>
                    <div style={{flex:1, minWidth:0}}>
                      <div style={{display:"flex", gap:8, alignItems:"center", flexWrap:"wrap", marginBottom:2}}>
                        <span style={{fontSize:13, fontWeight:700, color:TH.text}}>{dest}</span>
      ...
      {/* Photo lightbox */}
      {openPhoto && (
        <div onClick={() => setOpenPhoto(null)} style={{position:"fixed", inset:0, background:"rgba(0,0,0,0.95)", zIndex:10000, display:"flex", alignItems:"center", justifyContent:"center", padding:20, cursor:"pointer"}}>
          <img src={openPhoto} alt="" style={{maxWidth:"100%", maxHeight:"100%", borderRadius:8}} />
        </div>
      )}
    </div>
  );
...
function describeDest(r) {
  if (r.destination_pool_name) return ` ${r.destination_pool_name}`;
  if (r.destination_department_name) return ` ${r.destination_department_name}`;
