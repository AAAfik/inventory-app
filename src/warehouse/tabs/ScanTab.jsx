...
export default function ScanTab({ TH, lang = "en", isMobile, isAdmin, onChanged }) {
  ...
  return (
    <div>
      ...
      {/* Big scan button */}
      <div style={{background:TH.bgCard, border:`1px solid ${TH.border}`, borderRadius:14, padding:24, textAlign:"center", marginBottom:16}}><div style={{fontSize:60, marginBottom:12}}></div><div style={{fontSize:16, fontWeight:800, color:TH.text, marginBottom:6, fontFamily:"'Playfair Display', Georgia, serif"}}>
          {L.scanTabTitle || 'Scan asset barcode'}
        </div><div style={{fontSize:12, color:TH.textMuted, marginBottom:16}}>
          ...
          style={{
            ...
            color:TH.onDeep, padding:"14px 28px", fontSize:15, fontWeight:800, cursor:"pointer",
            fontFamily:"inherit", boxShadow:"0 2px 10px rgba(184,147,90,0.3)",
          }}
        >{L.scanBtn || 'Open scanner'}</button></div>

      {busy && <div style={{padding:16, textAlign:"center", color:TH.textMuted}}>{L.loading || 'Loading…'}</div>}

      {error && (
        <div style={{background:TH.dangerBg, border:`1px solid ${TH.danger}55`, borderRadius:10, padding:"12px 14px", color:TH.danger, fontSize:13, marginBottom:14}}>
          {error}
        </div>
      )}
      ...
      {/* Not found */}
      {notFoundValue && !busy && (
        <div style={{background:"rgba(232,122,44,0.1)", border:"1px solid rgba(232,122,44,0.4)", borderRadius:12, padding:16, marginBottom:14}}><div style={{fontSize:14, fontWeight:800, color:"#E67A2C", marginBottom:6}}>
            {L.scanNotFound || 'Not found'}
          </div><div style={{fontSize:12, color:TH.text}}><span style={{fontFamily:"monospace", background:TH.bgInput, padding:"2px 8px", borderRadius:4}}>{notFoundValue}</span>
      ...
      {/* Found asset card */}
      {found && !busy && (
        <div style={{background:TH.bgCard, border:`2px solid ${TH.accent || '#B8935A'}`, borderRadius:14, padding:16, marginBottom:14}}><div style={{fontSize:11, fontWeight:700, color:TH.accent, textTransform:"uppercase", letterSpacing:"0.05em", marginBottom:8}}>
            ...
            <div style={{flex:1, minWidth:0}}><div style={{fontSize:15, fontWeight:800, color:TH.text}}>{found.name}</div><div style={{fontSize:11, color:TH.textDim, fontFamily:"monospace", marginTop:2}}>{found.asset_no}{found.barcode ? ` · ${found.barcode}` : ''}</div><div style={{marginTop:6, display:"flex", gap:6, flexWrap:"wrap"}}>
          ...
          >{L.scanOpenDetail || 'Open details'}</button></div>
      )}

      {/* Recent scans this session */}
      {recentScans.length > 0 && (
        <div style={{background:TH.bgCard, border:`1px solid ${TH.border}`, borderRadius:12, padding:14}}><div style={{fontSize:11, fontWeight:700, color:TH.textMuted, textTransform:"uppercase", letterSpacing:"0.05em", marginBottom:8}}>
            {L.scanRecent || 'Recent scans'}
          </div>
          {recentScans.map((s, i) => (
            <div key={i} style={{display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 0", borderBottom: i < recentScans.length - 1 ? `1px solid ${TH.border}` : "none"}}><div><div style={{fontSize:12, fontFamily:"monospace", color:TH.text}}>{s.code}</div><div style={{fontSize:11, color:s.name ? TH.textMuted : "#E67A2C"}}>{s.name || (L.scanNotFound || 'Not found')}</div></div><div style={{fontSize:10, color:TH.textDim}}>{s.at.toLocaleTimeString()}</div></div>
          ))}
        </div>
      )}
    </div>
