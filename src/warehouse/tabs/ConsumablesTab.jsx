...
export default function ConsumablesTab({ TH, lang = "en", isMobile, isAdmin }) {
  ...
  return (
    <div>
      ...
      <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14, flexWrap:"wrap", gap:10}}><div style={{display:"flex", gap:4, background:TH.bgInput, borderRadius:9, padding:3}}><button onClick={() => setView("stock")} style={tabBtn(TH, view === "stock")}><Icon name="boxes" size={14} />Stock</button><button onClick={() => setView("items")} style={tabBtn(TH, view === "items")}><Icon name="ledger" size={14} />Items catalog</button></div><div style={{display:"flex", gap:6, flexWrap:"wrap"}}><button onClick={() => setReceivePreset("")} style={btnGreen()}><Icon name="arrowDown" size={14} />Receive</button><button onClick={() => setDispensePreset("")} style={btnGold()}><Icon name="arrowUp" size={14} />Dispense</button>
          {isAdmin && <button onClick={() => setEditItem({})} style={btnOutline(TH)}><Icon name="plus" size={14} />New item</button>}
        </div></div>

      {/* ─── Filters ─── */}
      <div style={{display:"grid", gridTemplateColumns:isMobile?"1fr":"2fr 1fr auto", gap:8, marginBottom:16}}><input value={search} onChange={e => setSearch(e.target.value)} placeholder={L.searchItem || "Search item name, code, category…"} style={inp(TH)} />
        {view === "stock" && (
          <select value={whFilter} onChange={e => setWhFilter(e.target.value)} style={inp(TH)}><option value="all">{L.allWarehouses || "All warehouses"}</option>
        ...
        {view === "stock" ? (
          ...
          }}><Icon name="warning" size={13} />{L.lowOnly || "Low only"}</button>
        ) : (
          isAdmin && (
            <label style={{display:"flex", alignItems:"center", gap:6, padding:"9px 12px", cursor:"pointer", color:TH.textMuted, fontSize:12}}><input type="checkbox"checked={showInactive} onChange={e => setShowInactive(e.target.checked)} />
              Show inactive
            </label>
          )
        )}
      ...
      {view === "stock" && (loading ? (
        ...
        <div style={{display:"flex", flexDirection:"column", gap:8}}>
          {rows.map(({ item, warehouse, qty, low, breakdown }) => (
            ...
            }}><div style={{flex:1, minWidth:0, cursor:"pointer"}} onClick={() => setHistoryItem(item)}><div style={{fontSize:13, fontWeight:700, color:TH.text}}>{item.name}</div><div style={{fontSize:10, color:TH.textDim}}>
                  ...
                  <div style={{display:"flex", gap:4, marginTop:5, flexWrap:"wrap"}}>
                    {breakdown.map(b => (
                      <span key={b.code} style={{fontSize:9, color:TH.textMuted, background:TH.bgInput, border:`1px solid ${TH.border}`, padding:"1px 6px", borderRadius:3}}>
                        {b.code}: <b style={{color:TH.text}}>{b.qty}</b></span>
                    ))}
                  </div>
                )}
              </div><div style={{textAlign:"right"}}><div style={{fontSize:18, fontWeight:800, color: low ? TH.warn : TH.text, lineHeight:1}}>
                  {qty} <span style={{fontSize:10, color:TH.textDim, fontWeight:500}}>{item.unit || ''}</span></div>
                {low && <div style={{fontSize:9, color:TH.warn, fontWeight:700}}>{L.low || "LOW"} ({L.min || "min"} {item.min_qty})</div>}
              </div><div style={{display:"flex", gap:4, flexShrink:0}}><button onClick={() => setReceivePreset(item.id)} title="Receive stock"style={{background:"transparent", border:`1px solid ${TH.ok}55`, borderRadius:6, color:TH.ok, padding:"6px 10px", cursor:"pointer", fontSize:14, fontWeight:700, fontFamily:"inherit", display:"flex", alignItems:"center", justifyContent:"center"}}><Icon name="arrowDown" size={14} /></button><button onClick={() => setDispensePreset(item.id)} title="Dispense"style={{background:"transparent", border:`1px solid ${TH.accent}55`, borderRadius:6, color:TH.accent, padding:"6px 10px", cursor:"pointer", fontSize:14, fontWeight:700, fontFamily:"inherit", display:"flex", alignItems:"center", justifyContent:"center"}}><Icon name="arrowUp" size={14} /></button><button onClick={() => setHistoryItem(item)} title="Movement history"style={{background:"transparent", border:`1px solid ${TH.border}`, borderRadius:6, color:TH.textMuted, padding:"6px 10px", cursor:"pointer", fontSize:12, fontFamily:"inherit"}}></button>
                {isAdmin && <button onClick={() => setEditItem(item)} title="Edit item"style={{background:"transparent", border:`1px solid ${TH.border}`, borderRadius:6, color:TH.textMuted, padding:"6px 10px", cursor:"pointer", fontSize:12, fontFamily:"inherit", display:"flex", alignItems:"center", justifyContent:"center"}}><Icon name="edit" size={14} /></button>}
              </div></div>
          ))}
        </div>
      ))}
      ...
      {view === "items" && (loading ? (
        ...
        <div style={{display:"grid", gridTemplateColumns:isMobile?"1fr":"repeat(auto-fill, minmax(280px, 1fr))", gap:10}}>
          {catalogItems.map(item => (
            <div key={item.id} style={{
            ...
            }}><div style={{display:"flex", justifyContent:"space-between", gap:8, marginBottom:6}}><div style={{flex:1, minWidth:0, cursor:"pointer"}} onClick={() => setHistoryItem(item)}><div style={{fontSize:14, fontWeight:700, color:TH.text}}>{item.name}</div><div style={{fontSize:10, color:TH.textDim, marginTop:2}}>
              ...
              <div style={{display:"flex", gap:6, flexWrap:"wrap", marginTop:8}}><span style={chip(TH)}>Unit: <b style={{color:TH.text}}>{item.unit}</b></span>
              ...
              </div><div style={{display:"flex", gap:4, marginTop:10}}><button onClick={() => setReceivePreset(item.id)} style={{flex:1, background:"transparent", border:`1px solid ${TH.ok}55`, borderRadius:6, color:TH.ok, padding:"6px", cursor:"pointer", fontSize:11, fontWeight:700, fontFamily:"inherit"}}><Icon name="arrowDown" size={14} />Receive</button><button onClick={() => setDispensePreset(item.id)} style={{flex:1, background:"transparent", border:`1px solid ${TH.accent}55`, borderRadius:6, color:TH.accent, padding:"6px", cursor:"pointer", fontSize:11, fontWeight:700, fontFamily:"inherit"}}><Icon name="arrowUp" size={14} />Dispense</button><button onClick={() => setHistoryItem(item)} style={{flex:1, background:"transparent", border:`1px solid ${TH.border}`, borderRadius:6, color:TH.textMuted, padding:"6px", cursor:"pointer", fontSize:11, fontWeight:600, fontFamily:"inherit"}}>History</button></div></div>
          ))}
        </div>
      ))}
    </div>
...
function btnGreen() {
  return { display:"inline-flex", alignItems:"center", gap:6, justifyContent:"center", background:TH.ok, border:"none", borderRadius:9, color:TH.onDeep, padding:"9px 16px", cursor:"pointer", fontSize:12, fontWeight:800, fontFamily:"inherit" };
}
function btnGold() {
  return { display:"inline-flex", alignItems:"center", gap:6, justifyContent:"center", background:TH.deep, border:"none", borderRadius:9, color:TH.onDeep, padding:"9px 16px", cursor:"pointer", fontSize:12, fontWeight:800, fontFamily:"inherit" };
}
function btnOutline(TH) {
  return { display:"inline-flex", alignItems:"center", gap:6, justifyContent:"center", background:"transparent", border:`1px solid ${TH.accent}`, borderRadius:9, color:TH.accent, padding:"9px 16px", cursor:"pointer", fontSize:12, fontWeight:700, fontFamily:"inherit" };
}
function inp(TH) {
