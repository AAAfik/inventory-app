      ...
      <div style={{display:"flex", gap:6, marginBottom:12, overflowX:"auto"}}>
        ...
        {Object.entries(STATUS_META).map(([k, m]) => (
          <PillBtn key={k} TH={TH} on={statusFilter === k} onClick={() => setStatusFilter(k)}>
            <Icon name={m.icon} size={15} /> {m.label} <Count on={statusFilter === k}>{statusCounts[k] || 0}</Count>
          </PillBtn>
        ))}
      </div>
                ...
                <div style={{display:"flex", gap:12, padding:14}}>
                  ...
                  <div style={{flex:1, minWidth:0}}>
                    <div style={{display:"flex", justifyContent:"space-between", gap:8, marginBottom:4}}>
                      ...
                      <span style={{fontSize:9, color:sm.color, fontWeight:700, textTransform:"uppercase", whiteSpace:"nowrap", flexShrink:0}}><Icon name={sm.icon} size={15} /> {sm.label}</span>
                    </div>
